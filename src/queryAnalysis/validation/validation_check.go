...
func checkQueryStores(databaseDetails []models.DatabaseDetailsDto) bool {
	...
	for _, database := range databaseDetails {
		if database.IsQueryStoreOn {
			log.Debug("Query store for this database is turned on: %s", database.Name)
			allQueryStoresOff = false
		} else {
			log.Debug("Query store disabled for database %s. Turn on with: ALTER DATABASE %s SET QUERY_STORE = ON;", database.Name, database.Name)
		}
	}
